...
let greaterThen = number[5] > number[4]; //false
console.log(`greaterThen:}${greaterThen}`);

// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = number[1] >= number[2];

console.log(`lessThenAndEqual:${lessThenAndEqual}`);
let greaterThenAndEqual;

// 동등(등급이나 정도가 같다) → boolean
number.push('10'); //[10, -4, 6, 101, -90, 10_000,"10"]
let equal = number[0] == number[6];
console.log(`equal: ${equal}`);

// 다름(같지 않음) → boolean
let inequal = number[0] !== number[6];

console.log(`inequal : ${inequal}`); // true

// 동일(똑같다) → boolean
let same = number[0] === number[6]; //야 !!!!!! 진짜 같아?
